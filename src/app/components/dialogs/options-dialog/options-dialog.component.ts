...
export class OptionsDialogComponent implements OnInit, AfterViewInit {
  ...
  deleteIdpUrl(id) {
    ...
    }, 'Delete IdP URL', 'Cancel', 'double-modal');
  }
  ...
  deleteAwsProfile(id: string) {
    ...
    // Get only names for display
    let sessionsNames = sessions.map(s => `<li><div class="removed-sessions"><b>${s.sessionName}</b> - <small>${(s as AwsIamRoleFederatedSession).roleArn ? ((s as AwsIamRoleFederatedSession).roleArn.split('/')[1] ? (s as AwsIamRoleFederatedSession).roleArn.split('/')[1] : 'Invalid Role ARN') : 'No Role'}</small></div></li>`);
    if (sessionsNames.length === 0) {
      sessionsNames = ['<li><b>no sessions</b></li>'];
    }
    ...
    }, 'Delete Profile', 'Cancel', 'double-modal');
  }
}
